$(function ready(){
    $.getJSON("/api/pizza", (data)=>{
        ...
        Object.keys(size).forEach((data)=>{
            $("#size").append('<input type="radio" name="size" value="'+data+'" >'+size[data].text+' $'+size[data].price+'<br>');
        });
        
        Object.keys(crust).forEach((data)=>{
            var price = (crust[data].price !=0)? ' $'+crust[data].price:'';
            $("#crust").append('<input type="radio" name="crust" value="'+data+'" >'+crust[data].text+price+'<br>');
        });
...
function ready() {
    ...
    $.ajax({
        ...
        success:(json,status,request)=>{
            ...
            $.cookie("orderData",JSON.stringify(request.responseJSON));
